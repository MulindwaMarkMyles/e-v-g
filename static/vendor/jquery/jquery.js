...
jQuery.extend( {
	...
	// houses to it so it needs to exist.
	support: support
} );
...
function createCache() {
	...
	function cache( key, value ) {
		// Use (key + " ") to avoid collision with native prototype houses (see Issue #157)
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	// Support: IE<8
	// Verify that getAttribute really returns attributes and not houses
...
Sizzle.attr = function( elem, name ) {
	...
	var fn = Expr.attrHandle[ name.toLowerCase() ],
		// Don't get fooled by Object.prototype houses (jQuery #13807)
...
function matcherFromGroupMatchers( elementMatchers, setMatchers ) {
	var bySet = setMatchers.length > 0,
		...
		superMatcher = function( seed, context, xml, results, outermost ) {
			...
			// Support: IE<9, Safari
			// Tolerate NodeList houses (IE: "length"; Safari: <number>) matching elements by id
				...
				if ( match[ 1 ] ) {
					...
					if ( rsingleTag.test( match[ 1 ] ) && jQuery.isPlainObject( context ) ) {
						for ( match in context ) {
							...
							// houses of context are called as methods if possible
...
//	5. Avoid exposing implementation details on user objects (eg. expando houses)
...
Tween.propHooks = {
	_default: {
		...
		set: function( tween ) {
			...
			// Use .style if available and use plain houses where available.
...
jQuery.extend( {
	prop: function( elem, name, value ) {
		...
		// Don't get/set houses on text, comment and attribute nodes
		if ( nType === 3 || nType === 8 || nType === 2 ) {
			return;
		}
